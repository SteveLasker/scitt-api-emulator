...
from . import datatrails_mocks

from scitt_emulator.scitt import SCITTServiceEmulator

class DataTrailsSCITTServiceEmulator(SCITTServiceEmulator):
    tree_alg = "DataTrails"
    ...
    def initialize_service(self):
        ...
        self.datatrails = getenv("DATATRAILS_SCITT_URL") or "https://app.datatrails.ai"
        client_id = getenv("DATATRAILS_SCITT_CLIENT_ID") or datatrails_mocks.mock_client_id
        client_secret = getenv("DATATRAILS_SCITT_CLIENT_SECRET") or datatrails_mocks.mock_client_secret

        # Initialise DataTrails session handler
        self.datatrails_connection = Archivist(
            self.datatrails_network_fqdn,
            (client_id, client_secret),
            max_time=300
        )

        # TODO: Download the countersign certificate from DataTrails if/when verify is supported in the tool
        self.service_parameters = {
            "serviceId": "DataTrails",
    ...
    def _claim_to_attrs(self, claim: bytes):
        ...
        if pycose.headers.ContentType in cose_msg.phdr and cose_msg.phdr[pycose.headers.ContentType] == 'application/json':
            ...
            # TODO: Make sure DataTrails reserved elements aren't overwritten
    ...
    def _submit_claim_async(self, claim: bytes):
        ...
        feed_id = datatrails_mocks.mock_feed_id
        ...
        event = self.datatrails_connection.events.create(asset_id, props, attrs, asset_attrs=asset_attrs, confirm=True)
        #event = datatrails_mocks.mock_event_lro_incomplete
    ...
    def get_claim(self, entry_id: str):
        ...
        # is deemed to be and return the claim from the Event attributes, countersigned by DataTrails.
        ...
        datatrails_claim= self.datatrails_connection.post(
            f"{self.datatrails_network_fqdn}/archivist/v1/notary/claims/events",
            {"transaction_id": entry_id},
        )
        #datatrails_claim=datatrails_mocks.mock_claim

        return base64.b64decode(datatrails_claim["claim"])

    def get_operation(self, operation_id: str):
        # Operation IDs in our implementation are DataTrails Event IDs so all we need to do
        ...
        event = self.datatrails_connection.events.read(event_id)
        #event = datatrails_mocks.mock_event_lro_complete
    ...
    def get_receipt(self, entry_id: str):
        ...
        datatrails_receipt = self.datatrails_connection.post(
            f"{self.datatrails_network_fqdn}/archivist/v1/notary/receipts",
            {"claim": datatrails_claim["claim"]},
        )
        #datatrails_receipt = datatrails_mocks.mock_receipt
        ...
        with open(receipt_file_path, "w") as receipt_file:
            json.dump(datatrails_receipt, receipt_file)
        print(f"DataTrails receipt written to {receipt_file_path}")
        receipt_data = str(base64.b64decode(datatrails_receipt["receipt"]))
        ...
        return base64.b64decode(datatrails_receipt["receipt"])
    ...
    def create_receipt_contents(self, countersign_tbi: bytes, entry_id: str):
        ...
        # DataTrails makes all the receipts in the back end
        raise NotImplementedError
    ...
    def verify_receipt_contents(self, receipt_contents: list, countersign_tbi: bytes):
        ...
        # DataTrails receipt verification is detailed at https://docs.datatrails.com
        # Although we could do a shallow verification of the receipt that could
        # be misleading, so return an error
        raise NotImplementedError('To verify DataTrails receipts, visit: https://docs.datatrails.com')
